import { Purchase } from '@models/integration';
...
const getDisplayCost = (cost: number): number => {
  return cost;
};
...
const parseCost = (bid: Purchase, settings: CostSettings, newLot: boolean): number => {
  ...
  return marblesAuc
    ...
    : getDisplayCost(convertedCost);
};

const getDisplayCostFormatted = (cost: number): number | string => {
...
const bidUtils = {
  parseCost,
  getDisplayCost: getDisplayCostFormatted,
  convertToMarble,
  getName,
  isValidCost,
};
